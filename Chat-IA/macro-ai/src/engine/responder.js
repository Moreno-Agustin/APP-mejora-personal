    ...
    try {
        ...
        if (intent.entities.includes("full_plan")) {
            ...
            if (currentUser && currentUser.sport && currentUser.sport !== updatedUser.sport) {
                ...
                return { text: generateFullPlanResponse(updatedUser, updatedPlan, advice) };
            }
            return { text: generateFullPlanResponse(updatedUser, plan, advice) };
        }
        // 5. Dynamic Responses
        if (thought.requiresClarification) {
            return { text: `[Intervención de IA] ${advice}`, options: thought.suggestions };
        }
        if (intent.category === "nutrition") {
            const foodList = plan.foods.map((f) => "• " + f).join("\n");
            return { text: `[Analizando Nutrición] ${advice}\n\nResumen de Macros:\n• Kcal: ${plan.nutrition.kcal}\n• Proteínas: ${plan.nutrition.protein}g\n• Carbs: ${plan.nutrition.carbs}g\n• Grasas: ${plan.nutrition.fat}g\n\nMenú Sugerido:\n${foodList}` };
        }
        if (intent.category === "training") {
            const methods = plan.training.methods.map((m) => "• " + m).join("\n");
            return { text: `[Protocolo de Entrenamiento] ${advice}\n\nFrecuencia: ${plan.training.sessions}/sem\nEnfoque: ${plan.training.focus}\n\nRutina Detallada:\n${methods}` };
        }
        if (intent.category === "habits") {
            const habitsList = [...plan.habits, ...plan.psychology].map((h) => "• " + h).join("\n");
            return { text: `[Optimización de Estilo de Vida]\n${advice}\n\nAcciones Inmediatas:\n${habitsList}` };
        }
        if (intent.category === "mood") {
            return { text: `[Apoyo Psicológico] ${advice}` };
        }
        return { text: advice };
    }
    catch (error) {
        console.error("AI Responder Error:", error);
        return { text: "Lo siento, mis procesadores están un poco saturados analizando tu plan. ¿Podrías intentar decirme algo más específico sobre tu nutrición o entrenamiento?" };
    }
}
