import { UserProfile, Sport } from "../types/types.js";
...
function generateFullPlanResponse(user: UserProfile, plan: any, advice: string): string {
...
${plan.foods.map((f: string) => "  " + f).join("\n")}
...
${plan.training.methods.map((m: string) => "  - " + m).join("\n")}

🧠 OPTIMIZACIÓN BIO-HÁBITOS:
${[...plan.habits, ...plan.psychology].map((h: string) => "• " + h).join("\n")}

🚀 MENSAJE DE TU COACH:
${advice}`;
}

export function respond(user: UserProfile, input: string): string {
    try {
        // 1. Understand
        const intent = analyze(input);

        // Ensure defaults for existing users
        if (user.energyLevel === undefined) user.energyLevel = 0.7;
        if (user.discipline === undefined) user.discipline = 0.5;
        if (user.stress === undefined) user.stress = 0.3;
        if (user.sleep === undefined) user.sleep = 7;
        if (user.interactionCount === undefined) user.interactionCount = 0;
        ...
        if (intent.category === "nutrition") {
            const foodList = plan.foods.map((f: string) => "• " + f).join("\n");
        ...
        if (intent.category === "training") {
            const methods = plan.training.methods.map((m: string) => "• " + m).join("\n");
        ...
        if (intent.category === "habits") {
            const habitsList = [...plan.habits, ...plan.psychology].map((h: string) => "• " + h).join("\n");
