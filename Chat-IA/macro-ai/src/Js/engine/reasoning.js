function getGoalSuggestions(sport) {
    const suggestions = {
...
export function reason(user, intent) {
    ...
    if (entities.includes("change") || wasAskedForSportChange) {
        ...
        if (mentionedSport) {
            suggestedChanges.sport = mentionedSport;
    ...
    if (entities.includes("elite_goal"))
        suggestedChanges.goal = "elite_performance";
    if (entities.includes("recreational_level"))
        suggestedChanges.level = "recreational";
    if (entities.includes("beginner_level"))
    ...
    if (user.interactionCount > 50 && user.stage === "initial")
        suggestedChanges.stage = "adaptation";
    if (user.discipline > 0.85 && user.stage === "adaptation")
        suggestedChanges.stage = "progress";
