...
export function buildTraining(user: UserProfile) {
    ...
    let sessions = 3;
    if (user.level === "intermediate" || user.level === "recreational") sessions = 4;
...
function generateTrainingDetails(user: UserProfile, sessions: number): string[] {
    const sport = user.sport || "gym";

    const trainingManifesto: Record<string, string[]> = {
        gym: [
