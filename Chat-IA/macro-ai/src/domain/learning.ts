import { UserProfile, Intent } from "../types/types.js";

export function learnFromInteraction(user: UserProfile, intent: Intent, text: string): UserProfile {
    ...
    if (intent.sentiment === "negative") updatedUser.energyLevel = Math.max(0.2, updatedUser.energyLevel - 0.1);
    if (intent.sentiment === "positive") updatedUser.energyLevel = Math.min(1, updatedUser.energyLevel + 0.1);
    ...
    if (updatedUser.history.length > 5) updatedUser.history.shift();
    ...
    if (updatedUser.interactionCount > 10 && updatedUser.stage === "initial") updatedUser.stage = "adaptation";
    if (updatedUser.interactionCount > 30 && updatedUser.stage === "adaptation") updatedUser.stage = "progress";
    if (updatedUser.interactionCount > 100 && updatedUser.stage === "progress") updatedUser.stage = "optimization";
